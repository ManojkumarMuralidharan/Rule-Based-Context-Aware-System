...
/*    Generated:  April 21st 2009 11:38:31 AM PDT                              */
/*      Machine:  10.16.139.101                                                */
/*       Source:  Global Cache                                                 */
/*     Location:  js/994qn6krufc48gok.pkg.js h:a64xkkxk                        */
/*       Locale:  nu_ll                                                        */
/*         Path:  js/994qn6krufc48gok.pkg.js                                   */


var s_account="facebookpoc"
...
s.linkInternalFilters="javascript:,facebook"
...
+"epa(v)}return ''");s.visitorNamespace="facebook"
s.dc=122
...
+"#A`W@b^Ws,`W`qs,$z,eo,_1_`b';#F=pg#L^2)`5!ss)`Is()",w=window,l=w.s_c_il,n=navigator,u=n.userAgent,v=n.appVersion,e=v.indexOf('MSIE '),m=u.indexOf('Netscape6/'),a,i,s;if(un){un=un.toLowerCase();if(l)for(i=0;i<l.length;i++){s=l[i];if(s._c=='s_c'){if(s.oun==un)return s;else if(s.fs&&s.sa&&s.fs(s.oun,un)){s.sa(un);return s}}}}
...
function seo_tracking_onload(channelAccount,defaultAccount,channelSampling,defaultSampling,pageName,pageType,visitorID,channel,server,locale_cookie,locale_browser,locale_ip,country,login){s.pageName=pageName;s.pageType=pageType;s.visitorID=visitorID;s.prop50=visitorID;s.channel=channel;s.server=server;s.prop18=locale_cookie;s.prop19=locale_browser;s.prop20=locale_ip;s.prop21=country;s.eVar24=login;_seo_tracking_init(s,channelAccount,channelSampling);_seo_tracking_write_onload(s);_seo_tracking_init(s,defaultAccount,defaultSampling);}
function _seo_tracking_init(s,channelAccount,channelSampling){s.sa(channelAccount);s.visitorSampling=channelSampling;}
function _seo_debug(msg){}
function _seo_tracking_write_onload(s){var s_code=s.t();if(s_code){document.write(s_code);}
_seo_debug(s.pageName);}
function seo_tracking_onclick(obj,channelAccount,defaultAccount,channelSampling,defaultSampling,what,where){var s=s_gi(channelAccount);switch(what){case'Login':_seo_tracking_login_event(s);break;case'Signup':_seo_tracking_signup_event(s);break;case'Find Friends':_seo_tracking_findfriends_event(s);break;default:return;break;}
_seo_tracking_init(s,channelAccount,channelSampling);_seo_tracking_send_onclick(s,obj,what,where);_seo_tracking_init(s,defaultAccount,defaultSampling);}
function _seo_tracking_send_onclick(s,obj,what,where){s.tl(obj,'o',what+': '+where);_seo_debug(what+': '+where);}
function _seo_tracking_login_event(s){s.linkTrackVars='events';s.linkTrackEvents='event1';s.events='event1';}
function _seo_tracking_signup_event(s){s.linkTrackVars='events,prop16,prop17';s.linkTrackEvents='event2';s.events='event2';var gender_element=ge('sex');var year_element=ge('birthday_year');if(gender_element&&year_element){var gender=gender_element.value==s.genderFemale?'Female':'Male';var bday=year_element.value;s.prop16=gender;s.prop17=bday;}}
function _seo_tracking_findfriends_event(s){s.linkTrackVars='events';s.linkTrackEvents='event3';s.events='event3';}

if (window.Bootloader) { Bootloader.done(["js\/994qn6krufc48gok.pkg.js"]); }